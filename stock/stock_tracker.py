#!/usr/bin/env python3
"""
Advanced Stock tracker script with color output, exports, and multiple timeframes
"""
...
DEFAULT_TICKERS = [
    ...
    'NFLX', 'GOOG', 'BRK.B', 'JNJ', 'V', 'WMT', 'JPM', 'DIS',
    'PYPL', 'ADBE', 'CRM', 'INTC', 'AMD'
]
...
def export_to_excel(df, filename=None):
    ...
    try:
        df.to_excel(filename, index=False, sheet_name='Stocks')
        print(f"{Fore.GREEN}Data exported to {filename}{Style.RESET_ALL}")
    except Exception as e:
...
def print_results(df, timeframe, title_suffix=""):
    ...
    if df is None or df.empty:
        print(f"{Fore.YELLOW}No valid stock data found{Style.RESET_ALL}")
        return
    
    print("\n" + "="*120)
    print(f"{Fore.CYAN}Stock Gains ({timeframe.upper()}) - {title_suffix}{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}{Style.RESET_ALL}")
    print("="*120)
    ...
    # Print top gainers
    print(f"\n{Fore.CYAN}Top 5 Gainers ({timeframe.upper()}){Style.RESET_ALL}:")
...
def main():
    parser = argparse.ArgumentParser(
        description='Advanced Stock Tracker - Track stocks with color output, exports, and multiple timeframes'
    )
    parser.add_argument('--tickers', nargs='+', help='Stock tickers to track')
    ...
    if args.all_timeframes:
        ...
        if args.export_csv or args.export_xlsx:
            ...
            if args.export_xlsx:
                filename = args.output or f"stocks_all_timeframes_{datetime.now().strftime('%Y%m%d_%H%M%S')}.xlsx"
                try:
            ...
            if args.export_csv:
                ...
                for tf, df in all_dfs.items():
                    filename = args.output or f"stocks_{tf}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv"
